...
export default function Header(props) {
    ...
    useEffect(() => {
     ...
     function AmountOfPeopleText() {
         ...
         let amountOfPeople = header.accountabilityExtraInfo.amountOfPeople;         
         return amountOfPeople.toString() === '1' ? '1 pessoa' : `${amountOfPeople} pessoas`; 
       
     }

     function BudgetForBreakfastText() {
        if(typeof header.accountabilityExtraInfo === 'undefined') return '';
        let budgetForBreakfast = header.accountabilityExtraInfo.budgetForBreakfast;           
    ...
    function CostCenterText(){
        ...
        const html = [];        
        let costCenters =  header.costCenters;
        costCenters.map((item) =>{              
            html.push(<div key={item.id}>{item.percentage}% - {item.name}</div>)
            return '';
        });
            ...
            <>
              <Row>
                    ...
                    <Row style={{paddingLeft:25}}>            
                        <Col className='header-title'>Reembolso #{header.id} - {header.justification}</Col>
                        <Col className='header-icon-edit'><FontAwesomeIcon  style={{color: "#FFFFFF"}} icon={faEdit} /></Col>
                    </Row>
                    ...
                    <Row style={{paddingLeft:25,marginTop:20}}>            
                        <Col md="8">
                            ...
                            <Col className='header-data-value'>
                                {header.accountabilityExtraInfo &&  Function.convertTimestampToDate(header.accountabilityExtraInfo.eventDate)}
                            </Col>
                            </Row>
                            <Row className='header-data-row'>
                            <Col md="3"  className='header-data-title'>
                        ...
                        <Col className='header-divisor'>
                            ...
                            <Col  className='header-data-value'>
                                ...
                                <select className='form-control header-form-select' defaultValue={''}>
                                        <option value="" disabled className='header-form-select-option-text'>Atribuir analista</option>
                                </select> 
                            </Col>              
                            </Row>
              ...
              <Row>
                <Col md="12"   style={{display: "flex" , justifyContent: 'flex-end'}}  >
                    <div className="btn-expend-add" onClick={()=> props.expenseFormSet()} >            
                        <button className="btn-expend-add-button"><FontAwesomeIcon  style={{color: "#748E97"}} icon={faReceipt} /> Adicionar&nbsp;Despesa</button>
                    </div>
                </Col>    
              </Row> 
            </>

      ...
      <Row>
        <Col md="2">
          <div className="circle circle-purple">
            <FontAwesomeIcon
              className="circle-icon circle-icon-purple"
              icon={faUsers}
            />
          </div>
          <div className="date">
            {Function.convertTimestampToDate(item.item.cardDate)}
          </div>
        </Col>
        <Col md="4" className="column">
          <div className="timeline-title">TIPO</div>
          <div className="timeline-value">
            Aprovação da Solicitação {item.item.author.name}
          </div>
        </Col>
        <Col md="4" className="column">
        ...
        <Col md="2" className="column">
          <div className="view-info">
            {' '}
            Ver aprovações <FontAwesomeIcon icon={faChevronDown} />
          </div>
        </Col>
      </Row>
    );
    ...
    return (
      <Row>
        ...
        <Col md="10" className="column">
          <div className="timeline-title">TIPO</div>
          <div className="timeline-value">
            Solicitação concluída por Backoffice Team
          </div>
        </Col>
      </Row>
    );
      ...
      <Row>
        ...
        <Col md="2" className="column">
          ...
          <div className="timeline-additional">
            {' '}
            Valor da nota: {item.item.expenseId}
          </div>
        </Col>
        <Col md="4" className="column">
          <div className="timeline-title">STATUS</div>
        ...
        <Col md="2" className="column">
          <div className="view-info">
            {' '}
            <FontAwesomeIcon icon={faReceipt} /> Ver nota fiscal{' '}
          </div>
        </Col>
      </Row>
    );
          ...
          <Col>
            <Card className="timeline-card">
              <Card.Body className="timeline-card-body">
                {item.cardType === 'EVALUATION' && <Evaluation item={item} />}
                {(item.cardType === 'ACCOUNTABILITY_SUBMITTED' ||
                  item.cardType === 'ACCOUNTABILITY_CREATED') && (
                  <Request item={item} />
                )}
                {item.cardType === 'EXPENSE' && <Hotel item={item} />}
              </Card.Body>
            </Card>
          </Col>
        </Row>
